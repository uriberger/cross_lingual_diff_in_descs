...
def to_concept_selection_page():
    if "language_selection_box0" in state:
        state.languages = [
            state[f"language_selection_box{i}"] for i in range(state.language_num)
        ]
...
def menu_page():
    ...
    st.markdown(
        "We use the CrossModal3600 dataset and extract the concepts mentioned in each caption, for each image in each language."
    )
    ...
    st.button(
        "Compare languages",
        key="compare_languages_button",
        on_click=to_two_languages_selection_page,
    )
...
def language_selection_page(language_num=1):
    ...
    for i in range(language_num):
        ...
        st.selectbox(
            label=button_text, key=f"language_selection_box{i}", options=lang_names
        )
    if state.concept is None:
        st.button(
            "Continue",
            key="language_page_continue_button",
            on_click=to_concept_selection_page,
        )
    else:
        st.button(
            "Continue",
            key="language_page_continue_button",
            on_click=to_language_by_concept_analysis_page,
        )
        st.markdown("OR")
        st.button(
...
def concept_selection_page():
    ...
    if len(parent2children[state.concept_selection_box]) > 0:
        st.button(
            "Select a sub concept",
            key="subconcept_selection_button",
            on_click=apply_concept_selection,
        )


def sub_concept_selection_page():
    language_str = (
        "" if state.languages is None else f', languages: {",".join(state.languages)}'
    )
    ...
    while len(queue) > 0:
        cur_concept = queue[0]
        nodes.append(
            Node(id=cur_concept, label=cur_concept, size=25, shape="rectangle")
        )
        queue = queue[1:]
    ...
    config = Config(
        width=600, height=400, directed=True, physics=True, hierarchical=True
    )

    state.concept = agraph(nodes=nodes, edges=edges, config=config)

    if state.languages is None:
        st.button(
            "Continue",
            key="sub_concept_selection_page_continue_button",
            on_click=to_language_selection_page,
        )
    else:
        st.button(
...
def by_single_language_concept_analysis_page():
    ...
    st.markdown(
        f"Out of the 3600 images in the datasets, {image_num} include captions mentioning {state.concept}"
    )
    if image_num > 0:
        prob_sum = sum(synset_to_image_prob[state.concept].values())
        prob_mean = prob_sum / image_num
        st.markdown(
            f"Each image is annotated by multiple annotators—some may mention the concept, while others don't. In these {image_num} images, the average fraction of annotators mentioning {state.concept} is {prob_mean:0.2f}"
        )
...
def by_two_languages_concept_analysis_page():
    lang_code1 = lang_name2code[state.languages[0]]
    synset_to_image_prob1, _, _ = get_synset_to_image_prob(f"xm3600_{lang_code1}")
    image_num1 = len(
        [x for x in synset_to_image_prob1[state.concept].values() if x > 0]
    )
    lang_code2 = lang_name2code[state.languages[1]]
    synset_to_image_prob2, _, _ = get_synset_to_image_prob(f"xm3600_{lang_code2}")
    image_num2 = len(
        [x for x in synset_to_image_prob2[state.concept].values() if x > 0]
    )
    st.subheader("Statistics")
    st.markdown(
        f"Out of the 3600 images in the datasets, {image_num1} include captions in {state.languages[0]} and {image_num2} include captions in {state.languages[1]} mentioning {state.concept}"
    )
    if image_num1 > 0:
    ...
    if prob_mean1 + prob_mean2 > 0:
        st.markdown(
            f"Each image is annotated by multiple annotators—some may mention the concept, while others don't."
        )
    if prob_mean1:
        st.markdown(
            f"For {state.languages[0]}, in these {image_num1} images, the average fraction of annotators mentioning {state.concept} is {prob_mean1:0.2f}"
        )
    if prob_mean2:
        st.markdown(
            f"For {state.languages[1]}, in these {image_num2} images, the average fraction of annotators mentioning {state.concept} is {prob_mean2:0.2f}"
        )

    with open(f"datasets/xm3600_{lang_code1}.json", "r") as fp:
        data1 = json.load(fp)
    iids = list(set([x["image_id"] for x in data1]))
    pval1 = stats.wilcoxon(
        [
            synset_to_image_prob1[state.concept][x]
            if x in synset_to_image_prob1[state.concept]
            else 0
            for x in iids
        ],
        [
            synset_to_image_prob2[state.concept][x]
            if x in synset_to_image_prob2[state.concept]
            else 0
            for x in iids
        ],
        alternative="greater",
    ...
    if pval1 < pval2:
        ...
        pval = pval2
    st.markdown(
        f"**Significance test**: When running the Wilcoxon signed-ranked test on the average fractions of annotators mentioning {state.concept} for each image, {first_lang} was found more likely to mention it ({not_str}significant, pvalue={pval:0.2f})"
    )

    iids1 = [
        x[0]
        for x in synset_to_image_prob1[state.concept].items()
        if x[1] == 1
        and (
            x[0] not in synset_to_image_prob2[state.concept]
            or synset_to_image_prob2[state.concept][x[0]] == 0
        )
    ]
    clicked1 = -1
    if len(iids1) > 0:
        st.subheader(
            f"Images where all annotators in {state.languages[0]} mentioned {state.concept} while none of the annotators in {state.languages[1]} did so:"
        )
        clicked1 = plot_clickable_images(iids1)

    iids2 = [
        x[0]
        for x in synset_to_image_prob2[state.concept].items()
        if x[1] == 1
        and (
            x[0] not in synset_to_image_prob1[state.concept]
            or synset_to_image_prob1[state.concept][x[0]] == 0
        )
    ]
    clicked2 = -1
    if len(iids2) > 0:
        st.subheader(
            f"Images where all annotators in {state.languages[1]} mentioned {state.concept} while none of the annotators in {state.languages[0]} did so:"
        )
    ...
    for lang in state.languages:
        ...
        for sample in samples:
            with st.container(border=True):
                ...
                st.markdown(
                    f'Identified concepts: {[("" + x[2] + "", x[3]) for x in sample["synsets"]]}'
                )

    st.button(
        "Back", key="image_back_button", on_click=to_language_by_concept_analysis_page
    )
...
def concept_analysis_across_all_languages_page():
    ...
    st.markdown(
        f"This plot presents the mean saliency of {state.concept} in each language."
    )
    st.markdown(
        f"The saliency of {state.concept} in a specific image in language $L$ is the fraction of annotators of the image in $L$ mentioning {state.concept}."
    )
    st.markdown(
        f"The overall saliency in $L$ is computed by averaging across all 3600 images in the CrossModal3600 dataset."
    )
